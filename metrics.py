...
#     print('Calculating SPS...')
    for idx, i in enumerate(range(len(next_items))):
        print(f'{str(idx).zfill(len(str(len(next_items))))}/{len(next_items)}', end='\r' )
...
def computeRecall(top_k_recommendations, recommendation_GT):
    '''
    The usual metrics for top-N recommendation,
    defined as the number of correct recommendations
    divided by the number of unique items in recommendation_GT.
    ...
    correct_recommendations_batch = np.zeros((bs))
...
#     print('Calculating Recall...')    
    ...
    for idx, b in enumerate(range(bs)):
        ...
        correct_recommendations = sum([1 for i in recommendations if i in recommendation_GT[b]])
        correct_recommendations_batch[b] = correct_recommendations 
    
    unique_recommendation_GT = [len(set(i)) for i in recommendation_GT]
    recall = np.mean(correct_recommendations_batch/unique_recommendation_GT)*100
    return recall

def computeItemCoverage(top_k_recommendations, recommendation_GT):
    '''
...
#     print('Calculating Item Coverage...')    
    ...
    for idx, b in enumerate(range(bs)):
        ...
        recommendations = top_k_recommendations[b]        
        unique_correct_recommendations = [i for i in list(set(recommendations)) if i in recommendation_GT[b]]
        # print('unique_correct_recommendations: ', unique_correct_recommendations)
        item_coverage.extend(unique_correct_recommendations)

    item_coverage = len(list(set(item_coverage)))
...
def computePopularRecommendations(top_k_recommendations, popular_songs):
    ...
    for idx, b in enumerate(range(bs)):
        recommendations = top_k_recommendations[b]
        popular_songs_count += sum([1 for i in recommendations if i in popular_songs])
...
def get_metrics(model, dataset):
    ...
    recall = computeRecall(top_k_recommendations, recommendation_GT)
    item_coverage = computeItemCoverage(top_k_recommendations, recommendation_GT)
#     user_coverage = computeUserCoverage(top_k_recommendations, recommendation_GT)
...
def generate_qualitative_results_on_handpicked_songs(model, write_name, handpicked_songs_list, dataset):
    ...
    for seq in handpicked_songs_list:
        seq_processed = [dataset.item2idx[i] for i in seq if i in dataset.vocab]
    ...
    for inp in handpicked_songs_list_processed:
        visualize_recommendations(model, dataset, write_name, tf.constant([inp]), dataset.NUM_ITEMS, num_recommendation_timesteps=NUM_RECOMMENDATION_TIMESTEPS)
...
#             tf.summary.scalar('user_coverage', user_coverage, step=count_dict['total_batches'])
    ...
    # if popular_recommendations < best_metrics_dict['best_popular_recommendations']:
    #     best_metrics_dict['best_popular_recommendations'] = popular_recommendations                    
    #     SAVE_MODEL = True
        
    # if user_coverage>best_metrics_dict['best_user_coverage']:
    #     best_metrics_dict['best_user_coverage'] = user_coverage
    #     SAVE_MODEL = True
    ...
    if QUALITATIVE_RESULTS_ON_HANDPICKED_SONGS:
        ...
        ##### ANIMESH  SONGS #####
        write_name = os.path.join(QUALITATIVE_RESULTS_DIR, f"epoch_{str(count_dict['epoch']).zfill(2)}_batch_{str(count_dict['total_batches']).zfill(8)}_animesh_songs.txt")
        handpicked_songs_df = pd.read_csv(QUALITATIVE_TEST_DATA_PATH)
        handpicked_songs_list = handpicked_songs_df['contentMetadata_contentId'].unique().tolist()
        generate_qualitative_results_on_handpicked_songs(model, write_name, handpicked_songs_list, dataset)            

        ##### SANIDHYA SONGS #####
        write_name_sanidhyas = os.path.join(QUALITATIVE_RESULTS_DIR, f"epoch_{str(count_dict['epoch']).zfill(2)}_batch_{str(count_dict['total_batches']).zfill(8)}_sanidhya_songs.txt")
        handpicked_songs_df = pd.read_csv(QUALITATIVE_TEST_DATA_PATH_SANIDHYA)
        handpicked_songs_list = handpicked_songs_df['song_id'].unique().tolist()
        generate_qualitative_results_on_handpicked_songs(model, write_name_sanidhyas, handpicked_songs_list, dataset)   
        
    else:
        print(f'\nGenerating Qualitative Results on {NUM_TEST_SAMPLES_QUALITATIVE} Test Samples ...')
        for chunk_idx, chunk in enumerate(pd.read_csv(TEST_DATA_PATH, chunksize=1, nrows=NUM_TEST_SAMPLES_QUALITATIVE)):  
            
            print(f'{str(chunk_idx).zfill(len(str(NUM_TEST_SAMPLES_QUALITATIVE)))}/{NUM_TEST_SAMPLES_QUALITATIVE}', end='\r' )
            song_id_list = chunk['song_id_list'].to_list()[0][1:-1].split(',')

            # Discarding the song_id s which are not present in the vocab
            song_id_list = [q for q in song_id_list if q in dataset.vocab]                           
            if len(song_id_list) < 2: continue

            cut = len(song_id_list)//2

            xs = [dataset.item2idx[x] for x in song_id_list[:cut]] # ignoring items that are not present in the vocab (train set)x
            xs = tf.constant([xs]) # idx and not item

            gt = [dataset.item2idx[x] for x in song_id_list[cut:]]
            visualize_recommendations(model, dataset, write_name, xs, dataset.NUM_ITEMS, gt, num_recommendation_timesteps=NUM_RECOMMENDATION_TIMESTEPS)
...
def get_top_k_recommendations(model, dataset, k=K):
    ...
    for chunk_idx, chunk in enumerate(pd.read_csv(TEST_DATA_PATH, chunksize=1, nrows=NUM_TEST_SAMPLES_QUANTITATIVE)):        
        ...
        xs = [dataset.item2idx[x] for x in song_id_list[:cut]] # ignoring items that are not present in the vocab (train set)
        xs_batch = np.array([xs])
        
        probs, _, _ = model(xs_batch, training=False, eval_mode=True)#.numpy()[0]
        probs = probs.numpy()[0] # np array (num_items)
        probs[xs] = 0
        
        top_k = np.argsort(-probs)[:k]
        top_k = [dataset.idx2item[i] for i in top_k]
        top_k_recommendations.append(top_k)
        
        next_item = song_id_list[cut]
        next_items.append(next_item)

        gt = [x for x in song_id_list[cut:]]
        recommendation_GT.append(gt)

    return top_k_recommendations, next_items, recommendation_GT


##### FOR GENERATING QUALITATIVE RESULTS ##### 
def visualize_recommendations(model, dataset, write_name, input_idx, num_items, gt=None, num_recommendation_timesteps=5):
    '''
    ...
    exclude = np.zeros([1, dataset.NUM_ITEMS]) #(1, num_items)
    ...
    for t in range(num_recommendation_timesteps):
        probs, state_h, state_c = model(inp, training=False, initial_state=initial_state, eval_mode=True) # probs (1, num_items)

        probs = probs*(1-exclude)             # probs (1, num_items)
        pred = tf.math.argmax(probs, axis=-1) # (1, )
        exclude[0, int(pred[0])] = 1 #(1, num_items)
...
def show_recommended_song_info(write_name, inp, dataset, recommendations, gt=None):    
    ...
    if PRINT_QUALITATIVE_RESULTS or WRITE_QUALITATIVE_RESULTS:
        ...
        if gt is not None:
            gt_info = get_info_from_item(gt[:10])
